import numpy as np
from feature_toolbox.datawrapper import Dataset, MAPPING
from feature_toolbox.datawrapper import dateformat
from feature_toolbox.dataset_daily import DailyDataset
from sklearn.metrics import r2_score
from sklearn.ensemble import RandomForestRegressor, ExtraTreesRegressor
from sklearn.cross_validation import StratifiedKFold
from feature_toolbox.utils.tools import database_loader
import datetime
import csv
...
class Forecaster(object):
    ...
    def cleanup_daterange(self,daterange):
        ...
        for j,date in enumerate(daterange):
            ...
            if (any(np.isnan(feature)) or np.isnan(target))==False:
    ...
    def check_lead_time_order(self,lead_times):
        for n in lead_times:
            if n[1]<n[0]:
                temp=n[0]
                n[0]=n[1]
    ...
    def lead_time2length(self,lead_times):
    ...
    def forecastwithaverage(self,date):
        ...
        decades=[]
        for date2 in dateformat.decadal_daterange(date.timedelta(self.lead_times[0][0]), date.timedelta(self.lead_times[0][1])):
        ...
        if output=='timeseries':
           ...
           return None
            ...
            for date in daterange:
                ...
                for date2 in dateformat.decadal_daterange(date.timedelta(self.lead_times[0][0]), date.timedelta(self.lead_times[0][1])):
                    decades.append(date2.decade_of_year)
                Error[relativeyear,date.decade_of_year-1]=np.absolute(self.forecast(date)-self.single_targetset(date))/(datasets[0].decadal_standard_deviation(decades))
            ...
            if self.shortterm_validation():
                ...
                for i,date in enumerate(daterange):
                    relativeyear=date.year-daterange[0].year
                    Error[relativeyear,date.decade_of_year-1]=np.absolute(predicted[i]-observed[i])/targetdiff.decadal_standard_deviation([date.decade_of_year])
    ...
    def csv(self, daterange, filename='result.csv'):
        ...
        for i, date in enumerate(daterange):
            ...
            for date2 in dateformat.decadal_daterange(date.timedelta(self.lead_times[0][0]), date.timedelta(self.lead_times[0][1])):
                decades.append(date2.decade_of_year)
            data=[date.year,date.decade_of_year,self.single_targetset(date),self.forecast(date),self.forecastwithaverage(date),targetset.decadal_standard_deviation(decades),targetdiff.decadal_standard_deviation(decades)]
...
def config2paramdict(config,database):
        ...
        datasets_in = []
        for type in config['datasets']:
            if type in MAPPING:
                datasets_in.append(Dataset(type, database))
            else:
                print type+' is not a valid datasettype: The following types can be used:'
                print MAPPING.keys()
                return None
...
if __name__ == '__main__':
    ...
    database=database_loader('/home/jules/Dropbox (hydrosolutions)/Hydromet/Forecasting_Dev/Forecasting_KG/database/pskem')
