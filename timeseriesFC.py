...
if __name__ == '__main__':
    ...
    KGcatchments=['AkSuu','AlaArcha','Alamedin','ChonKayindi','ChonKemin','KaraBalta','Kegety','Kochkor','Sokuluk']

    # Select Model
    model_type = ExtraTreesRegressor(n_estimators=100)
    ...
    for catchment in KGcatchments:
        # Select database (catchment)
        database = DatabaseLoader(
            '/home/jules/Dropbox (hydrosolutions)/Hydromet/Forecasting_Dev/flask_forecasting_serverKG/KGcatchments/'+catchment)

        # Select target and feature dataset(s) --> [target, feature1, feature2, ... ]
        datasets = [Dataset('runoff', database),Dataset('runoff',database).normalized(),Dataset('season',database).normalized()]

        # Select leadtimes for target and feature. negative:past/positive:future
        leadtimes = [[1, 3],[-4,-1],[1,1]]
